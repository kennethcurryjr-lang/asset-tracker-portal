...
function App() {
  ...
  const applyBulkClearLogs = async () => {
    if (!isAdmin || !window.confirm(`WARNING: PERMANENTLY delete all logs for all ${selectedDevices.length} selected devices?`)) return;
    try {
        await Promise.all(selectedDevices.map(async (id) => {
            const dev = assets.find(a => a.deviceId === id);
            if (!dev) return;
  ...
  const applyBulkNote = async () => {
    ...
    if (!window.confirm(`Broadcast this timeline log entry to all ${selectedDevices.length} selected devices?`)) return;
  ...
  const applyBulkSetHome = async () => {
    if (!window.confirm(`Set the current lock position as the home location anchor for all ${selectedDevices.length} selected devices?`)) return;
  ...
  return (
    <div style={appContainerStyle}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 0.33fr))', gap: '24px' }}>
          {filteredAssets.map(item => (
            <div key={item.deviceId} style={{ ...deviceCardStyle, backgroundColor: '#ffffff' }}>
                ...
                <div style={{ fontSize: '12px', color: getBatteryStatusColor(item.battery), fontWeight: '700' }}>{item.battery}% Battery</div>
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, backgroundColor: '#ffffff', borderTop: '1px solid #d2d2d7', padding: '20px 40px', transform: selectedDevices.length > 0 ? 'translateY(0)' : 'translateY(100%)', transition: 'transform 0.3s', zIndex: 4000 }}>
        <div style={{ maxWidth: '1140px', margin: '0 auto', display: 'flex', gap: '15px', justifyContent: 'flex-end' }}>
