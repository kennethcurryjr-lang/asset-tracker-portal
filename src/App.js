...
function App() {
  ...
  useEffect(() => {
    ...
    async function processPublicEscalationFetch() {
      ...
      try {
        ...
        if (!response.Items || response.Items.length === 0) { setShareError("Invalid configuration."); return; }
        const activeNode = response.Items.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp))[0];
        if (activeNode.shareExpires && Date.now() > activeNode.shareExpires) { setShareError("Expired."); return; }
      ...
      } catch (err) { setShareError("Failed."); } finally { setIsShareLoading(false); }
    }
  ...
  const fetchDevices = useCallback(async () => {
    ...
    try {
      ...
      const processed = await Promise.all(Object.keys(grouped).map(async (id) => {
        ...
        return { ...latest, deviceId: id, deviceNotes, tag: history.find(i => i.tag)?.tag || "", group: history.find(i => i.group)?.group || "", homeLat, homeLon, isServiceMode, lastServiceModeUser: history.find(i => i.lastServiceModeUser)?.lastServiceModeUser || "N/A", lastServiceModeTime: history.find(i => i.lastServiceModeTime)?.lastServiceModeTime || "N/A", isOffline, isGeofenceViolation, isLowBattery, estTimeRemaining, zip: loc.zip, city: loc.city, latitude: latest.latitude, longitude: latest.longitude, battery: latest.battery };
      }));
      setAssets(processed);
    } catch (err) { console.error(err); setDbError(err.message); }
  ...
  const toggleServiceMode = async (deviceId, timestamp, currentState) => {
    ...
    await Promise.all([updateAttribute(deviceId, timestamp, 'isServiceMode', newState, '#sm'), updateAttribute(deviceId, timestamp, 'lastServiceModeUser', user, '#lsu'), updateAttribute(deviceId, timestamp, 'lastServiceModeTime', time, '#lst'), addNote(deviceId, timestamp, logMsg)]);
  ...
  const deleteNote = async (deviceId, targetNote) => {
    ...
    await addNote(deviceId, targetNote.rowTimestamp, `🗑️ Log entry deleted`);
    ...
    if (response.Item?.notesList) {
        const updatedList = response.Item.notesList.filter(n => n.text !== targetNote.text || n.time.replace(/\s+/g, '') !== targetNote.time.replace(/\s+/g, ''));
        await docClient.send(new UpdateCommand({ TableName: "AssetTrackerData", Key: { deviceId, timestamp: targetNote.rowTimestamp }, UpdateExpression: "SET notesList = :ul", ExpressionAttributeValues: { ":ul": updatedList } }));
    }
  ...
  const applyBulkFactoryReset = async () => {
    if (!isAdmin || !window.confirm(`WARNING: PERMANENTLY wipe logs, reset names, and clear home/watchdog for all ${selectedDevices.length} selected devices?`)) return;
    try {
        ...
        setSelectedDevices([]); fetchDevices();
    } catch (err) { console.error(err); alert("Bulk reset failed."); }
  };

  const applyBulkClearLogs = async () => {
    if (!isAdmin || !window.confirm(`Delete all logs for ${selectedDevices.length} selected devices?`)) return;
    try {
        await Promise.all(selectedDevices.map(async (id) => {
            const dev = assets.find(a => a.deviceId === id);
            if (!dev) return;
        ...
        alert(`Successfully purged all logs.`); setSelectedDevices([]); fetchDevices();
    } catch (err) { alert("Bulk log deletion failed."); }
  };

  const executeLiveShareEscalation = async () => {
    if (!isAdmin) { alert("Security Violation."); setSharingAsset(null); return; }
    ...
    try {
      ...
      alert(`Secure Track link dispatched to ${shareEmail} for the next ${shareDuration} hours.`); setSharingAsset(null); setShareEmail(""); fetchDevices();
    } catch (err) { console.error(err); alert("System update failure."); }
  };

  const clearHomeLocation = async (deviceId, timestamp) => {
    await Promise.all([docClient.send(new UpdateCommand({ TableName: "AssetTrackerData", Key: { deviceId, timestamp }, UpdateExpression: "REMOVE homeLat, homeLon" })), addNote(deviceId, timestamp, `🚫 Home Anchor Cleared`)]);
  };

  const setHomeLocation = async (deviceId, timestamp, lat, lon) => {
    const logMsg = `📍 Home Anchor Set: ${lat.toFixed(4)}, ${lon.toFixed(4)}`;
    await Promise.all([updateAttribute(deviceId, timestamp, 'homeLat', lat, '#hl'), updateAttribute(deviceId, timestamp, 'homeLon', lon, '#hlon'), addNote(deviceId, timestamp, logMsg)]);
  ...
  const applyBulkGroup = async () => {
    ...
    alert(`Assigned group folder "${bulkGroupInput.trim()}" to ${selectedDevices.length} Kinetic Cards.`); resetAllInputs(); fetchDevices();
  };

  const applyBulkNote = async () => {
    if (!bulkNoteInput || !bulkNoteInput.trim()) return;
    if (!window.confirm(`Post log to ${selectedDevices.length} devices?`)) return;
    ...
    alert(`Broadcast log note to ${selectedDevices.length} Kinetic Card timelines.`); setBulkNoteInput(""); setSelectedDevices([]); fetchDevices();
  };

  const applyBulkSetHome = async () => {
    if (!window.confirm(`Set home location anchor for ${selectedDevices.length} selected devices?`)) return;
    ...
    alert(`Saved home target geofence anchors for ${selectedDevices.length} devices.`); setSelectedDevices([]); fetchDevices();
  };
  ...
  const applyBulkClearHome = async () => {
    ...
    alert(`Cleared home target anchors for ${selectedDevices.length} Kinetic Cards.`); setSelectedDevices([]); fetchDevices();
  };
  ...
  return (
    <div style={appContainerStyle}>
      <style>{`
        ...
        @media (max-width: 768px) { .card-split-columns-view { flex-direction: row !important; } .card-column-right-mapping { height: 120px !important; } }
      `}</style>
