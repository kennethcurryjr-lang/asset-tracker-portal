...
function App() {
  ...
  return (
    <div style={appContainerStyle}>
      ...
      {dbError && (
        <div style={{ backgroundColor: '#ff3b30', color: '#ffffff', padding: '12px 24px', fontSize: '14px', fontWeight: '600', textAlign: 'center' }}>⚠️ {dbError}</div>
      )}
                    ...
                    {historicalNotes.map((log, i) => (
                      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
                        ...
                        <div style={{ fontSize: '11px', fontWeight: '500' }}>
                          {log.text} <span style={{ fontSize: '9px', color: '#86868b' }}>• {log.time}</span>
                        </div>
                      </div>
                    ))}
                  </div>
