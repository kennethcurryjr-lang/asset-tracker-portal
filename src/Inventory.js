...
export default function Inventory({ user }) {
  ...
  const [isShrinkage, setIsShrinkage] = useState(false);
  ...
  const handleConfirmAction = async () => {
    ...
    const isActuallyShrinkage = isShrinkage || actionName.includes("Shrinkage");
    const finalNote = isActuallyShrinkage ? shrinkageNote : null;

    if (isActuallyShrinkage && (!finalNote || finalNote.trim().length < 5)) {
        alert(`🚨 COMPLIANCE STOP:

Transaction Aborted. You cannot process shrinkage without providing a valid reason (min 5 characters).

Please enter a note and try again.`);
        setIsShrinkage(true);
        return; // Physically halts the database save
    }
...
{auditLog.filter(log => !ledgerSearch || (log.orderNumber && log.orderNumber.toLowerCase().includes(ledgerSearch.toLowerCase())) || (log.flavor && log.flavor.toLowerCase().includes(ledgerSearch.toLowerCase()))).slice(0, 15).map((log, idx) => (
    ...
    <div style={{ fontSize: '18px', fontWeight: "600", letterSpacing: "-0.01em", color: 'var(--text-primary)', marginTop: '4px', lineHeight: '1.2', display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        {(() => {
            ...
            if (validLots.length > 1 && item.expiryDate === validLots[0].expiryDate) {
                return <span style={{ fontSize: '11px', padding: '3px 10px', borderRadius: '12px', fontWeight: '900', letterSpacing: '0.05em', whiteSpace: 'nowrap', animation: 'brute-force-blink 0.8s infinite', backgroundColor: 'rgba(48, 209, 88, 0.2)', border: '1px solid #30d158', display: 'inline-flex', alignItems: 'center' }}>
