    ...
    <div className="inventory-container" style={{ backgroundColor: "#1c1c1e", color: "#ffffff", minHeight: "100vh", padding: "32px", fontFamily: "-apple-system, BlinkMacSystemFont, sans-serif" }}>
      ...
      {lowStockItems.length > 0 && (
        <div style={{ backgroundColor: "rgba(255, 149, 0, 0.15)", border: "1px solid rgba(255, 149, 0, 0.4)", borderRadius: "16px", padding: "20px", marginBottom: "24px", display: "flex", flexWrap: "wrap", justifyContent: "space-between", alignItems: "center", gap: "16px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "16px" }}><span style={{ fontSize: "32px", lineHeight: "1" }}>⚠️</span><div><h4 style={{ margin: 0, color: "#ff9500", fontSize: "18px", fontWeight: "700" }}>Critical Stock Alert</h4><p style={{ margin: "4px 0 0 0", color: "#ffffff", fontSize: "14px", lineHeight: "1.4" }}>{lowStockItems.length} flavor lineup(s) have dropped below safe thresholds.</p></div></div>
          <button onClick={() => { window.location.href = `mailto:purchasing@csgroup.com?subject=URGENT: Restock Required`; }} style={{ backgroundColor: "#ff9500", color: "#ffffff", border: "none", padding: "12px 20px", borderRadius: "10px", fontWeight: "700", cursor: "pointer", whiteSpace: "nowrap" }}>✉️ Email Restock Report</button>
        </div>
      )}
      ...
      {pendingModeSwitch && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.85)", backdropFilter: "blur(10px)", zIndex: 10000, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "20px" }}>
          <div style={{ width: "100%", maxWidth: "420px", backgroundColor: "#1c1c1e", padding: "32px", borderRadius: "24px", border: "1px solid #3a3a3c", textAlign: "center", display: "flex", flexDirection: "column", gap: "20px", boxShadow: "0 20px 50px rgba(0,0,0,0.5)" }}>
            ...
            <p style={{ margin: 0, color: "#8e8e93", fontSize: "15px", lineHeight: "1.6" }}>Any barcodes scanned will now be <strong style={{ color: pendingModeSwitch === "ship" ? "#ff3b30" : "#34c759" }}>{pendingModeSwitch === "ship" ? "DEDUCTED from" : "ADDED to"}</strong> the live warehouse inventory.</p>
