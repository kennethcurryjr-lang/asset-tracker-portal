...
function Tools({ user }) {
  ...
  const fetchDB = async () => {
    ...
    try {
      ...
      console.log("DEBUG: User object keys:", Object.keys(user)); console.log("DEBUG: User attributes keys:", Object.keys(user.attributes || {})); console.log("DEBUG: Current session clientId:", uClientId);
  ...
  return (
    <div style={{ backgroundColor: '#f0f4f8', minHeight: '100vh', padding: '0 12px 100px 12px', color: '#0a1b35', fontFamily: '"SF Pro Display", sans-serif', maxWidth: '1440px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <style>{`
        ...
        .inspector-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 16px; }
        .asset-qr-container { flex-shrink: 0; margin-left: 20px; }
        .inspector-title-container { flex: 1; }
